    ...
    const animate = useCallback(() => {
        ...
        for (let y = 0; y < rows; y++) {
            for (let x = 0; x < cols; x++) {
                ...
                if (mouseInfluence > 0) {
                    waveValue +=
                        mouseInfluence * Math.sin(timeRef.current * 3) * 0.5;
                }

                // Add click wave influence
                const clickInfluence = getClickWaveInfluence(
                    posX,
                    posY,
                    currentTime,
                );
                ...
                if (Math.abs(waveValue) > 0.15) {
                    ...
                    const charIndex = Math.floor(
                        normalizedValue * CHARS.length,
                    );
                    const char =
                        CHARS[
                            Math.min(CHARS.length - 1, Math.max(0, charIndex))
                        ];
